...
public class TypeRateService {
    ...
    public List<TypeRateDTO> findAll() {
        ...
        return typeRates.stream()
                .map(typeRate -> mapToDTO(typeRate, new TypeRateDTO()))
                .toList();
    }

    public TypeRateDTO get(final Long id) {
        return typeRateRepository.findById(id)
                .map(typeRate -> mapToDTO(typeRate, new TypeRateDTO()))
                .orElseThrow(NotFoundException::new);
    }

    public Long create(final TypeRateDTO typeRateDTO) {
        final TypeRate typeRate = new TypeRate();
        mapToEntity(typeRateDTO, typeRate);
        return typeRateRepository.save(typeRate).getId();
    }

    public void update(final Long id, final TypeRateDTO typeRateDTO) {
    ...
    private TypeRateDTO mapToDTO(final TypeRate typeRate, final TypeRateDTO typeRateDTO) {
        typeRateDTO.setId(typeRate.getId());
        typeRateDTO.setName(typeRate.getName());
        return typeRateDTO;
    }

    private TypeRate mapToEntity(final TypeRateDTO typeRateDTO, final TypeRate typeRate) {
